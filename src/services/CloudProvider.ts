...
export class CloudProvider implements DataProvider {
  ...
  async getProfiles() { const rows = await getDocs(query(this.profiles(), orderBy('sortOrder'))); return rows.docs.map(x => profileFrom(x.id, x.data())); }
  ...
  async getBells(profileId: string) { const rows = await getDocs(query(collection(this.profiles(), profileId, 'bells'), orderBy('time'))); return rows.docs.map(x => bellFrom(profileId, x.id, x.data())); }
  ...
  async getSounds() { const rows = await getDocs(query(this.sounds(), orderBy('createdAt'))); return rows.docs.map(x => soundFrom(x.id, x.data())); }
  ...
  async getSettings() { const uid = userId(); const d = await getDoc(doc(firestore!, 'users', uid, 'settings', 'app')); return d.exists() ? { ...DEFAULT_SETTINGS, ...d.data() } as AppSettings : DEFAULT_SETTINGS; }
